...
public:
	static T element(const std::list < T >& s)
	{
		...
		int c = 0;
		for (typename std::list<T>::const_iterator itr = s.begin();
			itr != s.end(); itr++)
		{
			if (c == i) return *itr;
			c++;
	...
	static T element(const std::set < T >& s)
	{
		int i = Random::Int(0, s.size());
		int c = 0;
		for (typename std::set<T>::const_iterator itr = s.begin();
			itr != s.end(); itr++)
		{
			if (c == i) return *itr;
			c++;
	...
	static T element(const std::vector < T >& s)
	{
		int i = Random::Int(0, s.size());
		int c = 0;
		for (typename std::vector<T>::const_iterator itr = s.begin();
			itr != s.end(); itr++)
		{
			if (c == i) return *itr;
			c++;
	...
	static T chances(const std::map < T, float >& chs)
	{
		...
		int c = 0;
		for (typename std::map < T, float >::const_iterator itr = chs.begin();
			itr != chs.end(); itr++){
			values[c++] = itr->first;
		}
...
class BArray{
public:
	static std::vector<bool> myNot(std::vector<bool>&a, std::vector<bool>& result);
	static std::vector<bool> myOr(std::vector<bool>& a, std::vector<bool>& b, std::vector<bool>& result);
};
